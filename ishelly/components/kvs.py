...
class KVSSetParams(BaseModel):
    ...
    etag: Optional[str] = Field(None, description="Generated hash uniquely identifying the key-value pair")
...
class KVSSetResponse(BaseModel):
    etag: str = Field(..., description="Generated hash of the newly created or updated item")
...
class KVSGetManyParams(BaseModel):
    match: Optional[str] = Field(None, description="Pattern against which keys are matched")
...
class KVSGetManyResponse(BaseModel):
    items: Dict[str, Dict[str, Any]] = Field(..., description="JSON object containing a set of JSON objects")
...
class KVSListResponse(BaseModel):
    keys: Dict[str, str] = Field(..., description="Keys matched against the requested pattern")
...
req = KVSSetRequest(
    id=1,
    params=KVSSetParams(key="sample_key1", value="sample_value1")
)
...
req = KVSGetRequest(
    id=1,
    params=KVSGetParams(key="sample_key")
)
response = post(device_rpc_url, json=req.model_dump())


req = KVSGetManyRequest(
    id=1,
    params=KVSGetManyParams(key="sample_key*")
)
response = post(device_rpc_url, json=req.model_dump())
...
req = KVSDeleteRequest(
    id=1,
    params=KVSDeleteParams(key="sample_key", etag="0E8HnTToCT8e6BNK29wR1UGA==" )
)
response = post(device_rpc_url, json=req.model_dump())
