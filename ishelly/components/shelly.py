...
class ShellyGetDeviceInfoParams(BaseModel):
    ident: Optional[bool] = Field(None, description="Flag specifying if extra identifying information should be displayed")

class ShellyGetDeviceInfoRequest(JSONRPCRequest):
    method: str = Field("Shelly.GetDeviceInfo", description="Method to be invoked")
    params: ShellyGetDeviceInfoParams = Field(..., description="Parameters for the method")
...
class ShellyGetDeviceInfoResponse(BaseModel):
    ...
    auth_en: bool = Field(..., description="True if authentication is enabled, false otherwise")
    auth_domain: Optional[Union[str, None]] = Field(None, description="Name of the domain")
    discoverable: Optional[bool] = Field(None, description="If true, device is shown in 'Discovered devices'")
...
response = post(device_rpc_url, json=req.model_dump())
shelly_get_device_info_response = ShellyGetDeviceInfoResponse(**response.json()["result"])